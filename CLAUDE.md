...
- **Matriz de Treinamento por cargo — Fases 0 a 6 (completa)** — fecha o modelo SE Suite de Competência & Treinamento: a exigência de treinamento nasce do **cargo**, não da pessoa. Decisão que sustenta tudo: **exigência é derivada, evidência é gravada** — a exigência (quem deve treinar) não se armazena, calcula-se de `doc.treinamento.cargos` × quem ocupa esses cargos; a evidência (quem treinou) é gravada na coleção `treinamentos`, sempre carimbada com a versão do documento.
  ...
  - **Cenário de teste** (PR #125): `node scripts/seedTeste.js` popula o banco **local** espelhando o perfil oficial — 49 documentos (13/1/35), 12 cargos de fábrica, 22 pessoas, 66 evidências com cobertura desigual. `--limpar` reverte. Recusa rodar fora de banco local e só remove o que ele mesmo criou.
