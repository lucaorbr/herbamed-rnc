...
- **Matriz de Treinamento por cargo — Fases 0 a 4** — fecha o modelo SE Suite de Competência & Treinamento: a exigência de treinamento nasce do **cargo**, não da pessoa. Decisão que sustenta tudo: **exigência é derivada, evidência é gravada** — a exigência (quem deve treinar) não se armazena, calcula-se de `doc.treinamento.cargos` × quem ocupa esses cargos; a evidência (quem treinou) é gravada na coleção `treinamentos`, sempre carimbada com a versão do documento.
  ...
  - Fica para depois, se fizer sentido: **Fase 5** — lista de presença assinada pelo instrutor para treinamento presencial.
