...
- **Documentos — Formulário em Excel para o fornecedor RETIRADO do ar (v3.4.0)** — a emissão (`server/formularioXlsx.js` + endpoint `/api/documents/:id/formulario.xlsx` + seção 📗 no documento) entregava **arquivo defeituoso**, e a causa é arquitetural, não um bug pontual: o **ExcelJS não edita** o `.xlsx`, ele parseia para um modelo próprio e **reescreve o pacote do zero**. Tudo que esse modelo não representa não volta. Medido contra um formulário criado no Excel de verdade e reaberto no Excel de verdade:
  - **checkbox e demais controles de formulário somem** (`xl/ctrlProps/*` e o VML vão embora; `CheckBoxes().Count` cai de 1 para 0). Vale para ActiveX e macro.
  - **as fórmulas ficam silenciosamente erradas** — o pior dos três. O `spliceRows` desce a célula mas **não reescreve o texto da fórmula**, coisa que o Excel faz ao inserir linha. O `=SUM(E9:E13)` foi para a linha 18 ainda somando E9:E13, que ficaram vazias: com notas 5/4/5/3/1 o fonte dá **44** e o arquivo emitido dá **0**, sem erro nenhum na tela. Num SGQ é o pior tipo de falha — registro errado com cara de certo.
  - **fórmula arrastada nem gera**: shared formula estoura (`Shared Formula master must exist above and or left of clone`) e vira 422; formatação condicional derruba o `load` em certos arquivos; e o endpoint aceitava `.xls`, que o ExcelJS não lê.
...
- **RNC — PATCH apagava a RNC inteira + recuperação (v3.6.1)** — RNC aberta em 22/09 "ficou toda branca": sem número, descrição, severidade, prazos e assinatura; só sobrou status e histórico. **Causa:** a PR #145 (27/08, "aceitar PATCH para atualizar RNC") pôs `PATCH` na mesma condição do `PUT` em `/api/rncs/:id`, que **substitui** o registro pelo corpo. O bloco de merge do PATCH já existia logo abaixo e ficou inalcançável — a premissa da #145 ("o servidor só aceitava PUT/POST") estava errada. Todo `doUpdateRNC` manda patch parcial, então **qualquer ato de tratamento** esvaziava a RNC: mudança de status, assinatura do elaborador/RT, disposição, encerrar, contenção, Ishikawa, 5 Porquês, CAPA, prazo, eficácia, deliberação da RAC e o botão "Formulário p/ fornecedor" (o do caso de 22/09).
  - **Fix:** regra em `server/rncGravacao.js` (`mesclarPatchRNC`, `validarSubstituicaoRNC`, 6 testes). PATCH volta a mesclar; PUT que apagaria o `num` de uma RNC numerada é **recusado com 409** — corpo sem número é patch chegando pelo verbo errado, e falhar alto é melhor que esvaziar em silêncio. Leitura `FOR UPDATE` nos dois caminhos.
  - **Fix junto no `App.jsx`:** `doUpdateRNC`/`doDeleteRNC` tinham `useCallback(..., [])` — gravavam auditoria com o `auditLog` e o `rncs` da primeira renderização (sem usuário, sem `dadosAntes`). Agora `[rncs, auditLog]`.
...
- **Formulário em Excel para o fornecedor** — refazer sobre edição de ZIP (ver a entrada da v3.4.0 acima)
