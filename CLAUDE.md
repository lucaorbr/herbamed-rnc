...
- **Matriz de Treinamento por cargo — Fases 0 a 5 (completa)** — fecha o modelo SE Suite de Competência & Treinamento: a exigência de treinamento nasce do **cargo**, não da pessoa. Decisão que sustenta tudo: **exigência é derivada, evidência é gravada** — a exigência (quem deve treinar) não se armazena, calcula-se de `doc.treinamento.cargos` × quem ocupa esses cargos; a evidência (quem treinou) é gravada na coleção `treinamentos`, sempre carimbada com a versão do documento.
  ...
  - **Backlog levantado pela releitura do módulo** (não implementado): (1) **novo contratado nasce "atrasado"** — `statusCelula` conta o prazo de `doc.treinamento.desdeEm`, a data em que *a versão* entrou em vigor, igual para todos; quem é admitido hoje aparece atrasado há anos num POP antigo e o alerta da Fase 4 já cobra por e-mail. Não há data de admissão exposta (`publicUser` em `server/auth.js` não devolve `users.created_at`, que existe na tabela) — exige decisão de produto. (2) **evidência não pode ser anulada** — lançou no colaborador errado, não há caminho na tela; em BPF não se apaga, anula-se com justificativa e autor. (3) **PDF da matriz e ficha individual** da pessoa (hoje só CSV). (4) **avaliação de eficácia** do treinamento (prova/nota/verificação prática) — seria a Fase 6.
