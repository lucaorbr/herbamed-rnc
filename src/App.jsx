...
function POPsTab({ user, toast_, users }) {
  const T = useTheme(); const s = useS();
  ...
  const [form, setForm] = useState({
    titulo: "", categoria: "", setor: "Controle de Qualidade",
    responsavel: "", periodicidade: "", objetivo: "",
    aplicacao: "", materiais: [], etapas: [], criterio: "", obs: "",
  });
  const [novoMaterial, setNovoMaterial] = useState("");

  const isAdmin = user?.role === "admin" || user?.role === "keyuser" || user?.role === "rt";
  const setF = (k, v) => setForm(p => ({ ...p, [k]: v }));

  useEffect(() => {
    const t = setTimeout(() => setLoading(false), 3000);
    const unsub = subscribeCollection("pops", list => {
      clearTimeout(t);
      setPops(list.sort((a, b) => (b.criadoTs || 0) - (a.criadoTs || 0)));
      setLoading(false);
    });
    return () => { clearTimeout(t); unsub && unsub(); };
  }, []);

  // Gerar número sequencial POP
  const gerarNum = (categoria) => {
    const prefix = categoria.startsWith("CQ") ? "CQ" : categoria.startsWith("IPC") ? "IPC" : categoria.startsWith("Receb") ? "REC" : categoria.startsWith("Material") ? "MG" : categoria.startsWith("Embal") ? "EMB" : "GER";
    const ano = new Date().getFullYear();
    const existentes = pops.filter(p => p.num?.includes(`POP-${prefix}`)).length;
    return `POP-${prefix}-${ano}-${String(existentes + 1).padStart(3, "0")}`;
  };

  // IA — gerar rascunho do POP
  const gerarComIA = async () => {
    if (!form.titulo || !form.categoria) { alert("Informe o título e a categoria primeiro."); return; }
    setAiLoading(true);
    try {
      const prompt = `Você é um especialista em qualidade farmacêutica e nutracêutica. Crie um POP completo para:

Título: ${form.titulo}
Categoria: ${form.categoria}
Setor: ${form.setor || "Controle de Qualidade"}

Responda APENAS em JSON válido, sem markdown, sem explicações, no formato:
{
  "objetivo": "texto do objetivo",
  "aplicacao": "onde e quando aplicar",
  "periodicidade": "frequência de execução",
  "materiais": ["material 1", "material 2", "material 3"],
  "etapas": [
    {"titulo": "Nome da etapa", "descricao": "Descrição detalhada da etapa"},
    {"titulo": "Nome da etapa 2", "descricao": "Descrição detalhada"}
  ],
  "criterio": "critério de aceitação",
  "obs": "observações importantes"
}

Seja técnico, claro e específico para a indústria nutracêutica/farmacêutica brasileira.`;
      ...
      setForm(f => ({
        ...
        aplicacao: parsed.aplicacao || f.aplicacao,
        periodicidade: parsed.periodicidade || f.periodicidade,
        materiais: parsed.materiais || f.materiais,
        etapas: (parsed.etapas || []).map((e, i) => ({ id: Date.now() + i, titulo: e.titulo, descricao: e.descricao })),
        criterio: parsed.criterio || f.criterio,
        obs: parsed.obs || f.obs,
      }));
      toast_("Rascunho gerado pela IA!", "green");
    } catch (e) {
      toast_("Erro ao gerar com IA. Tente novamente.", "red");
    }
    setAiLoading(false);
  };

  // Etapas
  const addEtapa = () => setF("etapas", [...form.etapas, { id: Date.now(), titulo: "", descricao: "" }]);
  const setEtapa = (id, k, v) => setF("etapas", form.etapas.map(e => e.id === id ? { ...e, [k]: v } : e));
  const delEtapa = (id) => setF("etapas", form.etapas.filter(e => e.id !== id));
  ...
  const addMaterial = () => {
    ...
    setF("materiais", [...form.materiais, novoMaterial.trim()]);
    setNovoMaterial("");
  };
  const delMaterial = (i) => setF("materiais", form.materiais.filter((_, idx) => idx !== i));

  const salvar = async () => {
    if (!form.titulo) { alert("Informe o título do POP."); return; }
    if (!form.categoria) { alert("Selecione a categoria."); return; }
    const id = sel ? sel.id : Date.now();
    const num = sel ? sel.num : gerarNum(form.categoria);
    const pop = {
      id, num, ...form,
      status: "Rascunho",
    ...
    // Calcular status baseado nas assinaturas
    if (pop.assinaturaElaborador && pop.assinaturaRevisor && pop.assinaturaAprovador) pop.status = "Vigente";
    else if (pop.assinaturaElaborador || pop.assinaturaRevisor) pop.status = "Em revisão";
    await saveCollection("pops", String(id), pop);
    toast_(sel ? "POP atualizado!" : "POP criado!", "green");
    setView("lista"); setSel(null);
    resetForm();
  };

  const resetForm = () => setForm({ titulo: "", categoria: "", setor: "Controle de Qualidade", responsavel: "", periodicidade: "", objetivo: "", aplicacao: "", materiais: [], etapas: [], criterio: "", obs: "" });

  const deletar = async (id) => {
    if (!confirm("Excluir este POP?")) return;
    await deleteFromCollection("pops", String(id));
    setView("lista"); setSel(null);
    toast_("POP excluído.", "red");
  };

  const assinar = async (pop, tipo) => {
    if (!user.assinatura) { toast_("Cadastre sua assinatura no perfil primeiro.", "red"); return; }
    const campo = tipo === "elaborador" ? "assinaturaElaborador" : tipo === "revisor" ? "assinaturaRevisor" : "assinaturaAprovador";
    const cargo = tipo === "elaborador" ? "Elaborador" : tipo === "revisor" ? "Revisor" : "Aprovador";
    const updated = { ...pop, [campo]: { nome: user.name, cargo, crf: user.crf || "", img: user.assinatura, dataHora: `${tod()} ${new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}` } };
    if (updated.assinaturaElaborador && updated.assinaturaRevisor && updated.assinaturaAprovador) updated.status = "Vigente";
    else updated.status = "Em revisão";
    await saveCollection("pops", String(pop.id), updated);
    toast_(`POP assinado como ${cargo}!`, "green");
  };

  const exportPDF = (pop) => {
    const materiaisHTML = (pop.materiais || []).map(m => `<span style="font-size:11px;padding:3px 10px;border-radius:20px;background:#f0f0f0;color:#555;display:inline-block;margin:2px;">${m}</span>`).join("");
    const etapasHTML = (pop.etapas || []).map((e, i) => `
      <div style="display:flex;gap:12px;margin-bottom:10px;align-items:flex-start;">
        <div style="width:24px;height:24px;border-radius:50%;background:#1a4a2e;color:#fff;font-size:11px;font-weight:bold;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${i + 1}</div>
        <div style="flex:1;background:#f9f9f9;border-radius:6px;padding:8px 12px;border:1px solid #eee;">
          ${e.titulo ? `<div style="font-size:12px;font-weight:bold;margin-bottom:3px;">${e.titulo}</div>` : ""}
          <div style="font-size:11px;color:#555;line-height:1.5;">${e.descricao || ""}</div>
        </div>
      </div>`).join("");

    const assHTML = (ass, cargo) => ass ? `
      <div style="text-align:center;">
        <img src="${ass.img}" style="height:44px;max-width:160px;object-fit:contain;display:block;margin:0 auto 4px;"/>
        <div style="border-top:1px solid #ccc;padding-top:6px;font-size:11px;">
          <strong>${ass.nome}</strong><br/>${cargo}${ass.crf ? `<br/>${ass.crf}` : ""}<br/>
          <span style="color:#888;font-size:10px;">Assinado em ${ass.dataHora}</span>
        </div>
      </div>` : `
      <div style="text-align:center;">
        <div style="height:44px;display:flex;align-items:flex-end;justify-content:center;margin-bottom:4px;"><div style="border-bottom:1px solid #333;width:150px;"></div></div>
        <div style="border-top:1px solid #ccc;padding-top:6px;font-size:11px;">${cargo}<br/><span style="color:#bbb;font-size:10px;">Aguardando assinatura</span></div>
      </div>`;

    const html = `
      <div style="font-family:Arial,sans-serif;max-width:800px;margin:0 auto;">
        <div style="background:#1a4a2e;padding:16px 24px;display:flex;justify-content:space-between;align-items:center;">
          <div style="display:flex;align-items:center;gap:12px;">
            <img src="${LOGO_HERBAMED_POP}" style="width:40px;height:40px;border-radius:6px;object-fit:cover;"/>
            <div>
              <div style="color:#fff;font-size:13px;font-weight:bold;">${HERBAMED_POP.nome}</div>
              <div style="color:#9fd4b2;font-size:10px;">CNPJ: ${HERBAMED_POP.cnpj}</div>
            </div>
          </div>
          <div style="text-align:right;">
            <div style="color:#fff;font-size:12px;font-weight:bold;">Procedimento Operacional Padrão</div>
            <div style="color:#9fd4b2;font-size:11px;">N° ${pop.num}</div>
          </div>
        </div>
        <div style="background:#f0f7f3;padding:12px 24px;border-bottom:1px solid #eee;">
          <div style="font-size:15px;font-weight:bold;color:#1a4a2e;">${pop.titulo}</div>
          <div style="display:flex;gap:8px;margin-top:6px;">
            <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:#e1f5ee;color:#0f6e56;font-weight:bold;">${pop.categoria}</span>
            <span style="font-size:10px;padding:2px 8px;border-radius:10px;background:#f0f0f0;color:#666;">Elaborado em: ${pop.criadoEm}</span>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;border-bottom:1px solid #eee;">
          <div style="padding:10px 14px;border-right:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;margin-bottom:2px;">Setor</div><div style="font-size:12px;font-weight:bold;">${pop.setor || "—"}</div></div>
          <div style="padding:10px 14px;border-right:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;margin-bottom:2px;">Responsável</div><div style="font-size:12px;font-weight:bold;">${pop.responsavel || "—"}</div></div>
          <div style="padding:10px 14px;"><div style="font-size:10px;color:#888;text-transform:uppercase;margin-bottom:2px;">Periodicidade</div><div style="font-size:12px;font-weight:bold;">${pop.periodicidade || "—"}</div></div>
        </div>
        ${pop.objetivo ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:6px;">Objetivo</div><div style="font-size:12px;line-height:1.6;">${pop.objetivo}</div></div>` : ""}
        ${pop.aplicacao ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:6px;">Aplicação</div><div style="font-size:12px;line-height:1.6;">${pop.aplicacao}</div></div>` : ""}
        ${pop.materiais?.length ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:8px;">Materiais e equipamentos</div>${materiaisHTML}</div>` : ""}
        ${pop.etapas?.length ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:10px;">Etapas do procedimento</div>${etapasHTML}</div>` : ""}
        ${pop.criterio ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:6px;">Critério de aceitação</div><div style="font-size:12px;line-height:1.6;padding:8px 12px;background:#f0f7f3;border-left:3px solid #1a4a2e;">${pop.criterio}</div></div>` : ""}
        ${pop.obs ? `<div style="padding:12px 24px;border-bottom:1px solid #eee;"><div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:6px;">Observações</div><div style="font-size:12px;line-height:1.6;color:#555;">${pop.obs}</div></div>` : ""}
        <div style="padding:14px 24px;border-bottom:1px solid #eee;">
          <div style="font-size:10px;color:#888;text-transform:uppercase;font-weight:bold;margin-bottom:12px;">Assinaturas</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
            ${assHTML(pop.assinaturaElaborador, "Elaborador")}
            ${assHTML(pop.assinaturaRevisor, "Revisor")}
            ${assHTML(pop.assinaturaAprovador, "Aprovador")}
          </div>
        </div>
        <div style="padding:10px 24px;background:#f5f5f5;border-top:1px solid #eee;display:flex;justify-content:space-between;font-size:10px;color:#888;">
          <span>${HERBAMED_POP.nome} · CNPJ: ${HERBAMED_POP.cnpj}</span>
          <span>${HERBAMED_POP.endereco} · ${HERBAMED_POP.cidade} · CEP: ${HERBAMED_POP.cep}</span>
        </div>
      </div>`;
    const win = window.open("", "_blank");
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${pop.num}</title><style>@media print{body{margin:0}}</style></head><body>${html}<script>window.onload=()=>window.print();<\/script></body></html>`);
    win.document.close();
  };

  const statusColor = { "Vigente": T.accent, "Em revisão": T.yellow || "#f0c040", "Rascunho": T.text3 };
  const statusBg    = { "Vigente": T.accent + "18", "Em revisão": (T.yellow || "#f0c040") + "18", "Rascunho": T.border };
  const statusIcon  = { "Vigente": "✅", "Em revisão": "📝", "Rascunho": "⏳" };

  const filtrados = pops
    .filter(p => filtroCateg === "todas" || p.categoria === filtroCateg)
    .filter(p => filtroStatus === "todos" || p.status === filtroStatus)
    .filter(p => !busca || p.titulo?.toLowerCase().includes(busca.toLowerCase()) || p.num?.toLowerCase().includes(busca.toLowerCase()));

  // ── FORM ──
  if (view === "novo") return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <button style={s.btn} onClick={() => { setView("lista"); setSel(null); resetForm(); }}>← Voltar</button>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: T.text, margin: 0 }}>{sel ? "Editar POP" : "Novo POP"}</h2>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button style={{ ...s.btn, fontSize: 12, color: "#9d7fff", borderColor: "#9d7fff44", background: "#9d7fff11" }}
            onClick={gerarComIA} disabled={aiLoading}>
      ...
      <div style={s.card}>
        <SecTitle icon="📋" ch="Identificação" />
        <G2 ch={<>
          <F lbl="Título *" ch={<Inp placeholder="Ex: Determinação da Densidade Aparente" value={form.titulo} onChange={e => setF("titulo", e.target.value)} />} />
          <F lbl="Categoria *" ch={
            <Sel value={form.categoria} onChange={e => setF("categoria", e.target.value)}>
              <option value="">Selecione a categoria...</option>
              {POP_CATEGORIAS.map(c => <option key={c} value={c}>{c}</option>)}
            </Sel>
          } />
          <F lbl="Setor" ch={<Inp placeholder="Ex: Controle de Qualidade" value={form.setor} onChange={e => setF("setor", e.target.value)} />} />
          <F lbl="Responsável" ch={<Inp placeholder="Ex: Analista de CQ" value={form.responsavel} onChange={e => setF("responsavel", e.target.value)} />} />
          <F lbl="Periodicidade" ch={<Inp placeholder="Ex: A cada lote recebido" value={form.periodicidade} onChange={e => setF("periodicidade", e.target.value)} />} />
        </>} />
        <F lbl="Objetivo" ch={<TA rows={2} placeholder="Para que serve este procedimento..." value={form.objetivo} onChange={e => setF("objetivo", e.target.value)} />} />
        <F lbl="Aplicação" ch={<TA rows={2} placeholder="Onde e quando aplicar..." value={form.aplicacao} onChange={e => setF("aplicacao", e.target.value)} />} />
      </div>

      <div style={s.card}>
        <SecTitle icon="🧪" ch="Materiais e Equipamentos" />
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <Inp placeholder="Adicionar material ou equipamento..." value={novoMaterial} onChange={e => setNovoMaterial(e.target.value)}
            onKeyDown={e => e.key === "Enter" && addMaterial()} style={{ flex: 1 }} />
          <button style={{ ...s.btnA, fontSize: 12, padding: "6px 14px" }} onClick={addMaterial}>+ Add</button>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {form.materiais.map((m, i) => (
            <span key={i} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 20, background: T.surf, border: `1px solid ${T.border}`, color: T.text, display: "flex", alignItems: "center", gap: 6 }}>
              {m}
              <button onClick={() => delMaterial(i)} style={{ background: "none", border: "none", cursor: "pointer", color: T.text3, fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
            </span>
          ))}
          {form.materiais.length === 0 && <div style={{ fontSize: 12, color: T.text3 }}>Nenhum material adicionado. Digite e pressione Enter ou clique em "+ Add".</div>}
        </div>
      </div>

      <div style={s.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <SecTitle icon="📋" ch="Etapas do Procedimento" />
          <button style={{ ...s.btnA, fontSize: 11 }} onClick={addEtapa}>+ Adicionar etapa</button>
        </div>
        {form.etapas.length === 0 ? (
          <div style={{ textAlign: "center", padding: "1.5rem", color: T.text3, fontSize: 12 }}>Nenhuma etapa. Use "Gerar com IA" ou adicione manualmente.</div>
        ) : form.etapas.map((e, i) => (
          <div key={e.id} style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 10, padding: "12px 14px", marginBottom: 10, display: "flex", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <Inp placeholder="Título da etapa..." value={e.titulo} onChange={ev => setEtapa(e.id, "titulo", ev.target.value)} sx={{ marginBottom: 6, fontSize: 12, fontWeight: 600 }} />
              <TA placeholder="Descrição detalhada da etapa..." rows={2} value={e.descricao} onChange={ev => setEtapa(e.id, "descricao", ev.target.value)} sx={{ fontSize: 12 }} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <button onClick={() => moveEtapa(i, -1)} style={{ background: T.border, border: "none", borderRadius: 4, cursor: "pointer", padding: "2px 6px", color: T.text2, fontSize: 11 }}>↑</button>
              <button onClick={() => moveEtapa(i, 1)} style={{ background: T.border, border: "none", borderRadius: 4, cursor: "pointer", padding: "2px 6px", color: T.text2, fontSize: 11 }}>↓</button>
              <button onClick={() => delEtapa(e.id)} style={{ background: T.red + "22", border: "none", borderRadius: 4, cursor: "pointer", padding: "2px 6px", color: T.red, fontSize: 11 }}>✕</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={s.card}>
        <SecTitle icon="✅" ch="Critério de Aceitação e Observações" />
        <F lbl="Critério de aceitação" ch={<TA rows={2} placeholder="O que define se o procedimento está correto..." value={form.criterio} onChange={e => setF("criterio", e.target.value)} />} />
        <F lbl="Observações" ch={<TA rows={2} placeholder="Informações adicionais importantes..." value={form.obs} onChange={e => setF("obs", e.target.value)} />} />
      </div>

      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
        <button style={s.btn} onClick={() => { setView("lista"); setSel(null); resetForm(); }}>Cancelar</button>
        <button style={s.btnA} onClick={salvar}>Salvar POP ✓</button>
      </div>
    </div>
  );
  ...
  // ── DETALHE ──
  if (view === "detalhe" && sel) {
    const pSel = pops.find(p => p.id === sel.id) || sel;
    const podeAssElaborador = !pSel.assinaturaElaborador;
    const podeAssRevisor = pSel.assinaturaElaborador && !pSel.assinaturaRevisor;
    const podeAssAprovador = pSel.assinaturaRevisor && !pSel.assinaturaAprovador && (user.role === "rt" || user.role === "admin");

    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
          <button style={s.btn} onClick={() => { setView("lista"); setSel(null); }}>← Voltar</button>
          <h2 style={{ fontSize: 17, fontWeight: 700, color: T.text, margin: 0 }}>{pSel.num}</h2>
          <span style={{ padding: "3px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: statusBg[pSel.status], color: statusColor[pSel.status] }}>{statusIcon[pSel.status]} {pSel.status}</span>
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
            {podeAssElaborador && <button style={{ ...s.btnA, fontSize: 11 }} onClick={() => assinar(pSel, "elaborador")}>✍️ Assinar — Elaborador</button>}
            {podeAssRevisor && <button style={{ ...s.btnA, fontSize: 11 }} onClick={() => assinar(pSel, "revisor")}>🔎 Assinar — Revisor</button>}
            {podeAssAprovador && <button style={{ ...s.btnA, fontSize: 11, background: T.orange || "#ff9800" }} onClick={() => assinar(pSel, "aprovador")}>🔬 Assinar — Aprovador</button>}
            <button style={{ ...s.btn, fontSize: 11 }} onClick={() => exportPDF(pSel)}>🖨️ PDF</button>
            <button style={{ ...s.btn, fontSize: 11 }} onClick={() => { setSel(pSel); setForm({ titulo: pSel.titulo, categoria: pSel.categoria, setor: pSel.setor || "", responsavel: pSel.responsavel || "", periodicidade: pSel.periodicidade || "", objetivo: pSel.objetivo || "", aplicacao: pSel.aplicacao || "", materiais: pSel.materiais || [], etapas: pSel.etapas || [], criterio: pSel.criterio || "", obs: pSel.obs || "" }); setView("novo"); }}>✏️ Editar</button>
            {isAdmin && <button style={{ ...s.btnD, fontSize: 11 }} onClick={() => deletar(pSel.id)}>🗑️</button>}
          </div>
        </div>
        ...
        <div style={s.card}>
          <SecTitle icon="📋" ch="Identificação" />
          <G3 ch={<>
            <F lbl="Categoria" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.categoria}</div>} />
            <F lbl="Setor" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.setor || "—"}</div>} />
            <F lbl="Responsável" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.responsavel || "—"}</div>} />
            <F lbl="Periodicidade" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.periodicidade || "—"}</div>} />
            <F lbl="Elaborado por" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.criadoPor}</div>} />
            <F lbl="Data" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13 }}>{pSel.criadoEm}</div>} />
          </>} />
          {pSel.objetivo && <F lbl="Objetivo" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13, lineHeight: 1.6 }}>{pSel.objetivo}</div>} />}
          {pSel.aplicacao && <F lbl="Aplicação" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13, lineHeight: 1.6 }}>{pSel.aplicacao}</div>} />}
        </div>

        {pSel.materiais?.length > 0 && (
          <div style={s.card}>
            <SecTitle icon="🧪" ch="Materiais e Equipamentos" />
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {pSel.materiais.map((m, i) => (
                <span key={i} style={{ fontSize: 12, padding: "4px 12px", borderRadius: 20, background: T.surf, border: `1px solid ${T.border}`, color: T.text }}>{m}</span>
              ))}
            </div>
          </div>
        )}

        {pSel.etapas?.length > 0 && (
          <div style={s.card}>
            <SecTitle icon="📋" ch="Etapas do Procedimento" />
            {pSel.etapas.map((e, i) => (
        ...
        {(pSel.criterio || pSel.obs) && (
          <div style={s.card}>
            {pSel.criterio && <F lbl="Critério de aceitação" ch={<div style={{ padding: "10px 14px", background: T.accent + "12", borderLeft: `3px solid ${T.accent}`, borderRadius: "0 8px 8px 0", fontSize: 13, lineHeight: 1.6 }}>{pSel.criterio}</div>} />}
            {pSel.obs && <F lbl="Observações" ch={<div style={{ padding: "8px 10px", background: T.surf, borderRadius: 8, fontSize: 13, color: T.text2 }}>{pSel.obs}</div>} />}
          </div>
        )}
        ...
        <div style={s.card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            {[
              { campo: pSel.assinaturaElaborador, label: "Elaborador" },
              { campo: pSel.assinaturaRevisor, label: "Revisor" },
              { campo: pSel.assinaturaAprovador, label: "Aprovador" },
            ].map(({ campo, label }) => (
              <div key={label} style={{ textAlign: "center", padding: "1rem", background: T.surf, borderRadius: 10, border: `1px solid ${T.border}` }}>
                ...
                {campo ? (<>
                  <img src={campo.img} alt="Assinatura" style={{ height: 44, maxWidth: 160, objectFit: "contain", display: "block", margin: "0 auto 6px" }} />
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative" }}>
            <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: T.text3, fontSize: 13 }}>🔍</span>
            <input placeholder="Buscar POP..." value={busca} onChange={e => setBusca(e.target.value)} style={{ ...s.inp, paddingLeft: 30, width: 180, fontSize: 12 }} />
          </div>
          <Sel value={filtroCateg} onChange={e => setFiltroCateg(e.target.value)}>
        ...
        <button style={s.btnA} onClick={() => { setSel(null); resetForm(); setView("novo"); }}>+ Novo POP</button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "3rem", color: T.text3 }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📄</div>
          <div style={{ fontSize: 14 }}>Nenhum POP encontrado.</div>
          <div style={{ fontSize: 12, marginTop: 6 }}>Crie o primeiro procedimento operacional padrão!</div>
        </div>
      ) : filtrados.map(p => (
        <div key={p.id} className="rnc-row" onClick={() => { setSel(p); setView("detalhe"); }}
          style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${statusColor[p.status] || T.border}`, borderRadius: 10, padding: "12px 16px", marginBottom: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ width: 36, height: 36, borderRadius: 8, background: T.accentDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>📄</div>
          <div style={{ flex: 1, minWidth: 150 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{p.num} — {p.titulo}</div>
            <div style={{ fontSize: 11, color: T.text2, marginTop: 2 }}>{p.categoria} · {p.criadoEm} · {p.criadoPor}</div>
          </div>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {p.assinaturaElaborador && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 12, background: T.accent + "18", color: T.accent }}>✍️ Elaborador</span>}
            {p.assinaturaRevisor && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 12, background: T.accent + "18", color: T.accent }}>🔎 Revisor</span>}
            {p.assinaturaAprovador && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 12, background: (T.orange || "#ff9800") + "18", color: T.orange || "#ff9800" }}>🔬 Aprovador</span>}
          </div>
          <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: statusBg[p.status], color: statusColor[p.status], flexShrink: 0 }}>
